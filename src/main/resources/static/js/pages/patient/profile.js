/* ================================================
   profile.js  –  Patient profile logic
   ================================================ */
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    if (fDob && profileData.dateOfBirth) fDob.value = profileData.dateOfBirth;
    if (fGender && profileData.gender)
      fGender.value = profileData.gender.toLowerCase();
    if (fPhone && profileData.phoneNumber)
      fPhone.value = profileData.phoneNumber;
    if (fAddress && profileData.address) fAddress.value = profileData.address;
  ...
  const fDobInput = $("f-dob");
  const fGenderInput = $("f-gender");
  const fPhoneInput = $("f-phone");
  const fAddressInput = $("f-address");

  btnEdit?.addEventListener("click", async () => {
    if (editing) {
      const dto = {};
      if (fNameInput) dto.name = fNameInput.value;
      if (fDobInput) dto.dateOfBirth = fDobInput.value;
      if (fGenderInput) dto.gender = fGenderInput.value;
      if (fPhoneInput) dto.phoneNumber = fPhoneInput.value;
      if (fAddressInput) dto.address = fAddressInput.value;
      ...
      try {
        ...
        if (fDobInput) fDobInput.readOnly = true;
        if (fGenderInput) fGenderInput.disabled = true;
        if (fPhoneInput) fPhoneInput.readOnly = true;
        if (fAddressInput) fAddressInput.readOnly = true;
      ...
      if (fDobInput) fDobInput.readOnly = false;
      if (fGenderInput) fGenderInput.disabled = false;
      if (fPhoneInput) fPhoneInput.readOnly = false;
      if (fAddressInput) fAddressInput.readOnly = false;
      fNameInput?.focus();
    }
  });
